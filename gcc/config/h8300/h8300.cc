/* Subroutines for insn-output for Renesas H8/300.
...
/* Mark all the subexpressions of the PARALLEL rtx PAR as
   ...
   dwarf2out:dwarf2out_frame_debug_expr ignores sub-expressions of a
	       ...
	       unsigned int count, struct shift_info *info)
{
  ...
  switch (info->alg)
    ...
    case SHIFT_LOOP:
      ...
      info->cc_inline = shift_one[cpu_type][shift_type][shift_mode]_valid;
      goto end;
...
compute_a_shift_cc (rtx operands[3], rtx_code code)
{
  ...
  switch (info.alg)
    ...
    case SHIFT_SPECIAL:
      if (info.remainder == 0)
	return (info_special == OLD_CC_SET_ZN
		|| info_special == OLD_CC_SET_ZNV);

      /* Fall through.  */

    case SHIFT_INLINE:
      return (info_inline == OLD_CC_SET_ZN
	      || info_inline == OLD_CC_SET_ZNV);
