...
ix86_function_arg_regno_p (int regno)
{
  ...
  /* TODO: The function should depend on current function ABI but
     builtins would need updating then. Therefore we use the
     default ABI.  */
  ...
  if (TARGET_SEH)
    {
	 ...
	 area, see the SEH code in config/i386/winnt for the rationale.  */
...
/* Try machine-dependent ways of modifying an illegitimate address
   ...
   This macro is used in only one place: `memory_address' in explow.
   ...
   See comments by legitimize_pic_address in i386 for details.  */
