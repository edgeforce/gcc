/* Subroutines for insn-output for Windows NT.
...
gen_stdcall_or_fastcall_suffix (tree decl, tree id, bool fastcall)
{
  ...
  if (prototype_p (type))
	...
	return NULL_TREE;
	 ...
	 by convert_arguments in c-typeck or cp/typeck.  */
...
/* This hook behaves the same as varasm/assemble_name(), but
...
i386_pe_encode_section_info (tree decl, rtx rtl, int first)
{
  ...
  /* Mark the decl so we can tell from the rtl whether the object is
     dllexport'd or dllimport'd.  tree: merge_dllimport_decl_attributes
