...
s390_init_builtins (void)
{
  ...
  /* The uint64_type_node from tree is not compatible to the C99
     uint64_t data type.  What we want is c_uint64_type_node from
     c-common.  But since backend code is not supposed to interface
			  ...
			  rtx scratch)
{
  ...
  if (!s390_decompose_address (src, &ad)
      ...
      if (true_regnum (sum2) < 1 || true_regnum (sum2) > 15)
	{
	 ...
	 in reload, it should never happen (at least on s390) that
  ...
  if (TREE_CODE (args) == TREE_LIST)
    {
      ...
      /* Note: attribs:decl_attributes prepends the values from
	  ...
	  if (cl_options[opt].cl_uinteger)
	    {
		 ...
		 decode_cmdline_option () in opts-common.  */
