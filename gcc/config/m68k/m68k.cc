/* Subroutines for insn-output for Motorola 68000 family.
...
m68k_call_tls_get_addr (rtx x, rtx eqv, enum m68k_reloc reloc)
{
  ...
  /* FIXME: Unfortunately, emit_library_call_value does not
     ...
     calls:emit_library_call_value_1.
			...
			unsigned int)
{
  ...
  if (sched_cfv4_bypass_data.scale == 1)
    ...
    {
      /* haifa-sched: insn_cost () calls bypass_p () just before
  ...
  /* Don't try to issue INSN earlier than DFA permits.
     ...
     it should be moved to haifa-sched: insn_cost ().  */
		    ...
		    int n_insns ATTRIBUTE_UNUSED)
{
  ...
  if (sched_ib.enabled_p)
    /* haifa-sched: schedule_block () calls advance_cycle () just before
