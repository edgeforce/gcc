...
mmix_option_override (void)
{
  ...
  /* Don't bother with mmixal-compatible syntax if it's likely that a
     ...
     varasm:default_file_start for this triple.  See also
...
mmix_expand_prologue (void)
{
  ...
  if (MMIX_CFUN_HAS_LANDING_PAD)
      ...
      if (offset < 0)
	{
	 ...
	 like stack-pointer offsets.  Better: some hook into dwarf2out
...
#undef CCEND
  ...
  for (i = 0; i < ARRAY_SIZE (cc_convs); i++)
    {
      if (mode == cc_convs[i]_mode)
	{
	  for (j = 0; cc_convs[i].convs[j] != UNKNOWN; j++)
	    if (cc == cc_convs[i].convs[j])
	      {
		const char *mmix_cc
