...
/* Pointer to function (in rs6000-c) that can define or undefine target
   macros that have changed.  Languages that don't support the preprocessor
   don't link in rs6000-c, so we can't call it directly.  */
...
/* Try machine-dependent ways of modifying an illegitimate address
   ...
   This is used from only one place: `memory_address' in explow.
...
/* This is called from dwarf2out via TARGET_ASM_OUTPUT_DWARF_DTPREL.
...
rs6000_mode_dependent_address (const_rtx addr)
{
  switch (GET_CODE (addr))
    ...
    /* Auto-increment cases are now treated generically in recog.  */
    case PRE_MODIFY:
...
rs6000_adjust_priority (rtx_insn *insn ATTRIBUTE_UNUSED, int priority)
{
  ...
  if (insn_must_be_first_in_group (insn)
	...
	/* Attach highest priority to insn. This means that in
	   haifa-sched:ready_sort(), dispatch-slot restriction considerations
	...
	/* Increase priority of insn by a minimal amount. This means that in
	   haifa-sched:ready_sort(), only 'priority' (critical path)
...
rs6000_c_mode_for_suffix (char suffix)
{
  if (TARGET_FLOAT128_TYPE)
	...
	return (FLOAT128_IEEE_P (TFmode)) ? TFmode : KFmode;
	 ...
	 re-evalaute this decision.  Currently, c-lex only supports 'w' and
