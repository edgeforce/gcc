...
#undef ARC_OPT
  ...
  /* Set cpu flags accordingly to architecture/selected cpu.  The cpu
     specific flags are set in arc-common.  The architecture forces
	  ...
	  if (!final_sequence || XVECLEN (final_sequence, 0) < 2
	      ...
	      || simplejump_p (final_sequence->insn (0)))
	    {
		 ...
		 independently of its position in a delay slot.  */
	      fprintf (file, "%s%s",
		       code == '?' ? "." : "",
		       arc_condition_codes[arc_ccfsm_current]);
		 ...
		 and the only short conditional returns are jeq_s and jne_s.  */
	      if (code == '!'
		  && (arc_ccfsm_current == ARC_CC_EQ
		      || arc_ccfsm_current == ARC_CC_NE
		      || 0 /* FIXME: check if branch in 7 bit range.  */))
		output_short_suffix (file);
	    }
	  else if (code == '!') /* Jump with delay slot.  */
	    fputs (arc_condition_codes[arc_ccfsm_current], file);
	      ...
	      if (JUMP_P (jump) && INSN_ANNULLED_BRANCH_P (jump))
		{
		  if (INSN_FROM_TARGET_P (insn))
		    fprintf (file, "%s%s",
			     code == '?' ? "." : "",
			     arc_condition_codes[ARC_INVERSE_CONDITION_CODE (arc_ccfsm_current)]);
		  else
		    fprintf (file, "%s%s",
			     code == '?' ? "." : "",
			     arc_condition_codes[arc_ccfsm_current]);
 ...
 /* We can't predicate BRCC or loop ends.
  ...
  if (reverse
      ...
      if (succeed)
	{
	 ...
	 destroy this array, but final assumes that it remains intact
...
arc_arg_partial_bytes (cumulative_args_t cum_v, const function_arg_info &arg)
{
  ...
  /* ICEd at function:2361, and ret is copied to data->partial */
...
conditionalize_nonjump (rtx pat, rtx cond, rtx insn, bool annulled)
{
  ...
  /* dwarf2out:dwarf2out_frame_debug_expr doesn't know
     what to do with COND_EXEC.  */
  if (RTX_FRAME_RELATED_P (insn))
    {
      /* If this is the delay slot insn of an anulled branch,
	 dwarf2out:scan_trace understands the anulling semantics
